...
EMOTION_CONFIG = {
    ...
    "sad": {
        ...
        "vol_full": 0.28, "vol_ambient": 0.22,
    },
    "angry": {
        "intensity": "high",
        "music_prompt": (
    ...
    "disgust": {
        ...
        "vol_full": 0.25, "vol_ambient": 0.20,
    },
    "neutral": {
        "intensity": "low",
        "music_prompt": (
...
def generate_all_layers(scene_segments, emotion_segments, dom_env,
    ...
    # Normalize each layer to its target volume independently
    # This ensures music is always audible regardless of object sound loudness
    def norm_layer(layer, target):
        mx = np.abs(layer).max()
        if mx > 1e-6:
            return layer / mx * target
        return layer

    layer1 = norm_layer(layer1, 0.55)   # emotion music  — 55% of headroom
    layer2 = norm_layer(layer2, 0.15)   # environment BG —  15% of headroom
    layer3 = norm_layer(layer3, 0.80)   # object sounds  — 80% of headroom

    final = layer1 + layer2 + layer3
    mx    = np.abs(final).max()
    if mx>0: final = final/mx*0.90

    print(f"  Layer 1 (emotion music)  peak: {np.abs(layer1).max():.3f}")
    print(f"  Layer 2 (environment BG) peak: {np.abs(layer2).max():.3f}")
    print(f"  Layer 3 (object sounds)  peak: {np.abs(layer3).max():.3f}")
